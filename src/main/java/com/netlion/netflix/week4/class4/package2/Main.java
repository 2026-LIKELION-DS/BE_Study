package com.netlion.netflix.week4.class4.package2;
...
public class Main {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (choice.equals("1")) {
                register(scanner, members, partMap);
    ...
    private static void register(Scanner scanner, List<Role> members, Map<String, List<Role>> partMap) {
    ...
    private static void searchByPart(Scanner scanner, Map<String, List<Role>> partMap) {
        ...
        int index = 1;
        for (Role role : partMembers) {
            System.out.println(index + ". " + role.getName() + " (" + role.getRoleName() + ") - " + role.getSummary().substring(role.getSummary().indexOf("- ") + 2));
            index++;
        }
    }
}
